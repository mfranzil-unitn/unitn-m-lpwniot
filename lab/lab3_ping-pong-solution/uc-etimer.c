...
recv_unicast(struct unicast_conn *c, const linkaddr_t *from)
{
  ...
  printf("Node: recv from %02X:%02X Length = %u, RSSI = %ddBm, "
    ...
    from->u8[0], from->u8[1], packetbuf_datalen(), rssi, 
    msg.noise_floor, msg.number);
...
PROCESS_THREAD(unicast_process, ev, data)
{
  ...
  /* Allocate application process event */
  app_event = process_alloc_event();

#ifndef CONTIKI_TARGET_SKY
  /* In TI CC2650-based plafform, activate battery and temperature sensors */
