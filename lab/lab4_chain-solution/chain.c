...
send_msg()
{
  ...
  // pack seqn to network byte order (big-endian) 
  payload[0] = seqn >> 8;
  payload[1] = seqn;
...
recv_uc(struct unicast_conn *c, const linkaddr_t *from)
{
  ...
  seqn = ((uint16_t)payload[0]<<8) | payload[1]; // unpack from network byte order (big-endian) 
  hops = payload[2];

  size_t str_length = packetbuf_datalen() - 3;
  char str[str_length + 1];
  memcpy(str, packetbuf_dataptr() + 3, str_length);
  str[str_length] = '\0'; // add the terminating zero
...
static void
nbr_table_add(const linkaddr_t* addr)
{
  int i;
...
static bool
get_random_neighbor(linkaddr_t* addr)
{
  int num_nbr = 0;
  int i;
