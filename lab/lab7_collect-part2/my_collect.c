...
void uc_recv(struct unicast_conn *uc_conn, const linkaddr_t *from) {
   ...
   */
    ...
    if (conn->is_sink) {
        ...
        //4. Call the application recv callback to inform the application about the received data (app payload, source, hops)
        ...
        printf("my_collect: recv data from %02x:%02x hops %u\n",
               from->u8[0], from->u8[1], hdr.hops);

        //recv_cb(hdr.source, hdr.hops);
    } else {
        if (linkaddr_cmp(&conn->parent, &linkaddr_null)) {
            // The node is not connected
